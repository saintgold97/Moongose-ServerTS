...
router.get("/validate/:tokenVerify", async (req, res) => {
    const userVerify = await User.findOneAndUpdate({ verify: req.params.tokenVerify }, { $unset: { "verify": 1 } });
    userVerify ?
...
router.post(
    ...
    async ({ body }, res) => {
        try {
            ...
            if (userValid && !userValid.verify && (await bcrypt.compare(password, userValid.password!))
            ) {
